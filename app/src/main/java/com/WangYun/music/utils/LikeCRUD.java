...
import com.WangYun.music.domain.MusicListInfo;
...
public class LikeCRUD {
    ...
    public void likeAdd(Context context, int position){
        ...
        try {
            ...
            if(l == -1){
                Toast.makeText(context, "该歌曲已添加", Toast.LENGTH_SHORT).show();
            }else {
                Toast.makeText(context, "加入我喜欢成功", Toast.LENGTH_SHORT).show();
            }
        }catch (Exception e){}
        db.close();
    }
    public void likeDelete(Context context, int position){
        ...
        if(i == 0){
            ...
            Toast.makeText(context,info.getName()+"已从我喜欢中删除",Toast.LENGTH_SHORT).show();
        }
        db.close();
    }
